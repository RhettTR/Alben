...
int Counter::moveImage_x = 0;
int Counter::moveImage_y = 0;
...
{	
	...
	Overlay::hooverView->layout->setGeometry(QRect(0,0,0,0));
	
	
	Overlay::hooverView->grid->resize(Overlay::FlowLayout::layoutWidth, Overlay::FlowLayout::layoutHeight);
	
	int height = box > Overlay::FlowLayout::layoutHeight + 4 ? box : Overlay::FlowLayout::layoutHeight + 4;
	
	Overlay::hooverView->resize(box + Overlay::border + Overlay::FlowLayout::layoutWidth, height);
	...
	Overlay::hooverView->moveThis(pnt, 
								  box + Overlay::border + Overlay::FlowLayout::layoutWidth, 
								  height);	
...
void Counter::QtCounter::mouseDoubleClickEvent (QMouseEvent * e)
{
    ...
    if (e->button() == Qt::LeftButton)
    {
        ...
        Overlay::openView->updateImages(this->owner);
...
void Counter::setImage()
{
	...
	if (state.moved)
	{
		...
		int dx = margin + Counter::moveImage_x;
		int dy = margin + Counter::moveImage_y;
