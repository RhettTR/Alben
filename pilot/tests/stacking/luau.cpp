...
extern "C" {
	...
	{	
		...
		int degrees = lua_tonumber(L, -3);
		const char *image = lua_tostring(L, -4);
		int id = lua_tonumber(L, -5);
		lua_pop(L, 5);
		printf("id image degrees %d %s %d %d %d\n", id, image, degrees, x, y);
		(void)new Counter(id, image, degrees, x, y);
	...
	static int set_counter_image_r(lua_State *L)
	{		
		if (lua_isstring(L, -1))
		{	
			const char *image = lua_tostring(L, -1);
	...
	static int rotate_counter_r(lua_State *L)
	{		
...
void Luau::doCreate(const char *fromId, const char *trait, const char *toId, int degrees, int x, int y)
{
...
void Luau::copyCounter(const char *fromId, const char *toId, int degrees, int x, int y)
{
...
void Luau::startVM()
{
	...
	lua_pushcfunction(L, set_counter_image_r, "set_counter_image_r");
	lua_setglobal(L, "set_counter_image_r");
	...
	lua_pushcfunction(L, rotate_counter_r, "rotate_counter_r");
	lua_setglobal(L, "rotate_counter_r");
